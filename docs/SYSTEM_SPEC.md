# AQI Predictor — System Specification
...
| Stage |  | Evidence |
|-------|--------|----------|
| Data Collection |  | Open-Meteo API, 4-year range (Aug 2022 – Aug 2026) + hourly live collection |
...
| EDA |  | 4 Jupyter notebooks |
| Feature Engineering | | 58 features |
| Feature Store |  | Hopsworks PRIMARY, 107,067 rows (single store, no local backup) |
| Feature View |  | Target label designation |
| Model Training |  | 4 models on complete 4-year data from Hopsworks |
| Model Evaluation |  | MAE + RMSE + R² across all horizons |
| Model Selection |  | XGBoost selected (best composite score) |
| Model Registry |  | Hopsworks Model Registry (XGBoost v4) |
| CI/CD |  | 487 tests pass, lint clean |
| Deployment |  | Render (API) + Streamlit Cloud (Dashboard) |
| Automation |  | Hourly collection (verified live in Hopsworks) + daily retraining |
...
Both scheduled workflows use these under **Settings → Secrets and variables → Actions**:
