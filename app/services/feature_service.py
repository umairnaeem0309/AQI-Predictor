...
class FeatureService:
    ...
    def get_features(
        ...
        if self.primary_store:
            try:
                features = self._fetch_from_store(self.primary_store, city, feature_names)
                return features
                ...
                if self.fallback_enabled and self.fallback_store:
                    ...
                    try:
                        features = self._fetch_from_store(self.fallback_store, city, feature_names)
                        return features
                    ...
                    raise FeatureStoreConnectionError(f"Primary store failed: {e}")
    ...
    def _fetch_from_store(
        ...
        if features_df.empty:
            raise FeatureStoreConnectionError(f"No features found for city: {city}")
    ...
    def validate_feature_schema(
        ...
        if missing:
            raise FeatureSchemaMismatchError(f"Missing required features: {missing}")
        return True
